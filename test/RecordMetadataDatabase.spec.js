...
} from '../compat-tests/RecordMetadata';
...
describe('RecordMetadataDatabase', () => {
  ...
  it.concurrent(
    'should parallelize tests across browsers and retain order of test results',
    async () => {
  ...
  it.concurrent('should have exact match', async () => {
    ...
    const insertedMetadataSet = new Set(
      insertedMetadata.map(e => {
        const item = delete e.astNodeType;
        return item;
      })
    );
